    ...
    public class ComplexFluidInteractor : BaseFluidInteractor
    {
        ...
        public override void FluidUpdate()
        {
            foreach (Transform floater in floaters)
            {
                ...
                if (difference < 0)
                {
                    // --- 1. Calculate buoyancy and scale it down ---
                    Vector3 buoyancy = Vector3.up * floatStrength * Mathf.Abs(difference) * Physics.gravity.magnitude * volume * fluid.density * 0.5f;

                    // --- 2. Add reduced turbulence if enabled ---
                    if (simulateWaterTurbulence)
                    {
                        buoyancy += GenerateTurbulence() * 0.3f;
                    }

                    // --- 3. Apply buoyancy force at floater position ---
                    rb.AddForceAtPosition(buoyancy, floater.position, ForceMode.Force);

                    // --- 4. Apply linear velocity damping ---
                    rb.AddForceAtPosition(rb.linearVelocity * (dampeningFactor / floaters.Count) * volume, floater.position, ForceMode.Force);
                }
            }

            // --- 5. Damp angular velocity to prevent spinning ---
